...
export default async function binance(req, res) {
  try {
    ...
    Object.keys(total).filter(currency => total[currency] != 0).forEach((name) => {
      let currency = new Currency(name, Math.round(total[name] * 10000) / 10000, 0);

      if (Object.keys(tickers).includes(name + "/USDT")) {
        currency.average = tickers[name + "/USDT"].average;
        currency.value = Math.round((currency.total * currency.average) * 1000) / 1000;
      }
